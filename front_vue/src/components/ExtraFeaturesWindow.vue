<template>
    <div class="container">
        <div class="box_extra" v-show="select_elm == undefined">
            <div class="row" @click="handleClick('add_place')">Создать новое место в выбранной точки</div>
            <div class="row" @click="handleClick('login')">Вход</div>
            <div class="row" @click="handleClick('registration')">Регистрация</div>
            <div class="row" @click="handleClick('about')">О проекте</div>
        </div>
        <div class="box_select_elm">
            <AddPlace v-show="select_elm === 'add_place'" @hidden_elm="hidden_elm" />
        </div>
    </div>
</template>
